...
def train_lstm(batch_size=80,time_step=15,train_begin=2000,train_end=5800):
    ...
    with tf.Session() as sess:
        ...
        for i in range(201):
            ...
            print(i,loss_)
            if i % 200==0:
                print("saveing model：",saver.save(sess,'./model2/stock2.model',global_step=i))
...
def prediction(time_step=15):
    ...
    with tf.Session() as sess:
        ...
        #以折线图表示结果
        plt.figure()
        plt.plot(list(range(len(test_predict))), test_predict, 'ro',label = 'y_predict')
        plt.plot(list(range(len(test_y))), test_y,  'bo',label = 'y_test')
