...
if __name__=='__main__':
    ...
    X_train, y_train, X_test, y_test = load_data('./data/sp500.csv', seq_len, True)
    ...
    print("loading model ")
    #加载模型数据和weights  
    model = model_from_json(open('./model/series_lstm_model.json').read())    
    model.load_weights('./model/series_lstm_weights.h5')    
    ...
    open('./model/series_lstm_model.json','w').write(json_string)    
    model.save_weights('./model/series_lstm_weights.h5')    
